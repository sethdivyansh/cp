...
ll dfs(int i, int j, vector<vector<char>> mat, char c, ll len){
    ll n = mat.size();
    ll m = mat[0].size();

    if(i < 0 || j < 0) return len;
    if(i >= n || j >= m) return len;
    if(mat[i][j] != c+1) return len;

    len++;

    dfs(i-1,j-1,mat,c+1,len);
    dfs(i-1,j,mat,c+1,len);
    dfs(i-1,j+1,mat,c+1,len);
    dfs(i,j-1,mat,c+1,len);
    dfs(i,j+1,mat,c+1,len);
    dfs(i+1,j-1,mat,c+1,len);
    dfs(i+1,j,mat,c+1,len);
    dfs(i+1,j+1,mat,c+1,len);
...
void solve(){
    ...
    while(true){
        ...
        ll ans = 0;
        for(auto p: pos){
            ans = max(ans, dfs(p.first, p.second, mat, 'A', 0));
        }
